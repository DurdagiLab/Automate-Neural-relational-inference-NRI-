...
Dis=features[0][0][0:3,:]
D=np.zeros((406,406))
for i in range(406):
    for j in range(406):
        D[i,j]=(np.sum((Dis[:,i]-Dis[:,j])**2))**(1/2)
        
        
edges_results_visual = getEdgeResults(threshold=False)
edges_results_filter12=edges_results_visual
tresh=12
Sum=0
for i in range(406):
    for j in range(i+1,406):
        if D[i,j]<=tresh:
            Sum+=edges_results_visual[i,j]

            
for i in range(406):
    for j in range(406):
        if D[i,j]>tresh:
            edges_results_filter12[i,j]=0
            
print(f"NRI estimation for free energy score is = {Sum}")

mdict={"map":edges_results_visual}
savemat("map.mat", mdict)

mdict={"map":edges_results_filter12}
savemat("map_12.mat", mdict)
# Step 1: Visualize results
ax = sns.heatmap(edges_results_visual,
                 cmap="Blues")
plt.savefig('logs/probs_train.png', dpi=600)
# plt.show()
plt.close()

ax = sns.heatmap(edges_results_filter12,
                 cmap="Blues")
plt.savefig('logs/probs_train_tresh_12.png', dpi=600)
# plt.show()
plt.close()
