...
parser.add_argument('--pdb-end', type=int, default="100",
...
parser.add_argument('--train-interval', type=int, default=200,
        ...
        for line in lines:
            ...
            if(line.startswith("MODEL")):
                ...
                if (modelNum % interval == (window_choose+1)):
                    nflag = True
            elif (fin==0) and (line.startswith("ATOM") and words[2] == "CA" and int(words[4]) >= aa_start and int(words[4]) <= aa_end and flag):
                #print(modelNum,interval,window_choose)
                numStep = int(modelNum/interval)
            ...
            elif (fin==0) and (line.startswith("ATOM") and words[2] == "CA" and int(words[4]) >= aa_start and int(words[4]) <= aa_end and nflag):
                numStep = int(modelNum/interval)
                feature[numStep, 3, int(
            ...
            elif (fin==0) and (line.startswith("ENDMDL") and flag):
                flag = False
            elif (fin==0) and (line.startswith("ENDMDL") and nflag):
                nflag = False
    f.close()
    # print(feature.shape)
    return feature
...
def convert_dataset_md_single(MDfolder, startIndex, experiment_size, timestep_size, feature_size, num_residues, interval, pdb_start, pdb_end, aa_start, aa_end):
    ...
    for i in range(startIndex, experiment_size+1):
        ...
        for j in range(pdb_start, pdb_end+1):
            print(str(i)+" "+str(j))
            features.append(read_feature_MD_file_slidingwindow(MDfolder+pdb_file+'-'+str(
...
features, edges = convert_dataset_md_single(MDfolder, startIndex=1, experiment_size=1, timestep_size=20,
...
features_valid, edges_valid = convert_dataset_md_single(MDfolder, startIndex=1, experiment_size=1, timestep_size=20,
...
"""
print("Generate Test")
features_test, edges_test = convert_dataset_md_single(MDfolder, startIndex=1, experiment_size=1, timestep_size=20,
